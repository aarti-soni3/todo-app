            ...
            <div className="card rounded border" style={{ width: '22rem', marginLeft: '20px', marginBottom: '20px' }} data-bs-theme="dark">
                <div className="card-body" style={{ scrollbarWidth: 'thin', height: '500px', overflowY: 'auto', overflowX: 'hidden' }}>
                    <h5 className="card-title" style={{ marginBottom: '25px', width: '320px' }}>
                        {showUpdateBoard ? <UpdateBoard boardId={boardId} boardTitle={board.boardTitle} hideUpdateBoard={() => { setShowUpdateBoard(false) }} /> : board.boardTitle}
                    ...
                    <ul className="list-group" style={{ alignItems: 'start' }}>
                        {
                            Object.entries(tasks)
                                ...
                                .map(([taskId, task]) => (
                                    <TaskItem key={taskId} taskId={taskId} boardId={boardId} taskTitle={task.taskTitle} taskDescription={task.taskDescription} isCompleted={task.isCompleted} />
                                ))
                        }
                    </ul>
                    {showAddTaskItem ? <AddTaskItem key={boardId} boardId={boardId} hideAddTaskItem={() => { setShowAddTaskItem(false) }} /> : <div className="d-grid col-8 mx" style={{ position: 'relative', marginLeft: '2.8rem', marginBottom: '8px' }}>
